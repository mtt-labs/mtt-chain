...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=evmos \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(MTT_BINARY) \
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(MTT_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evmos:Z evmos/node "./mttd testnet init-files --v 4 -o /evmos --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
