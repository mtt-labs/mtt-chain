...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=evmos \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(BOYAA_BINARY) \
...
build-docker:
	...
	mkdir -p ./build/.mttd
	echo '#!/usr/bin/env bash' > ./build/mttd
	echo "IMAGE_NAME=${DOCKER_IMAGE}:${COMMIT_HASH}" >> ./build/mttd
	echo 'SCRIPT_PATH=$$(cd $$(dirname $$0) && pwd -P)' >> ./build/mttd
	echo 'docker run -it --rm -v $${SCRIPT_PATH}/.mttd:/home/evmos/.mttd $$IMAGE_NAME mttd "$$@"' >> ./build/mttd
	chmod +x ./build/mttd
...
localnet-start: localnet-stop localnet-build
	@if ! [ -f build/node0/$(BOYAA_BINARY)/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/evmos:Z evmos/node "./mttd testnet init-files --v 4 -o /evmos --keyring-backend=test --starting-ip-address 192.167.10.2"; fi
	docker-compose up -d
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\build\node0\mttd:/evmos\Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node1\mttd:/evmos\Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node2\mttd:/evmos\Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)\build\node3\mttd:/evmos\Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
else
	@docker run --rm -v $(CURDIR)/build/node0/mttd:/evmos:Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node1/mttd:/evmos:Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node2/mttd:/evmos:Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
	@docker run --rm -v $(CURDIR)/build/node3/mttd:/evmos:Z evmos/node "./mttd tendermint unsafe-reset-all --home=/evmos"
endif
