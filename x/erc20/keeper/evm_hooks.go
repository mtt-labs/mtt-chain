...
import (
	...
	"github.com/mtt-labs/mtt-chain/contracts"
...
func (h Hooks) AfterRecvPacket(ctx sdk.Context, receiver sdk.AccAddress, coin sdk.Coin) error {
	//TODO implement me
	return h.k.AfterRecvPacket(ctx, receiver, coin)
}

func (k Keeper) AfterRecvPacket(ctx sdk.Context, receiver sdk.AccAddress, coin sdk.Coin) error {
	//mock msg
	msg := &types.MsgConvertCoin{}
	msg.Sender = receiver.String()
	msg.Coin = coin
	addr, err := sdk.AccAddressFromBech32(receiver.String())
	if err != nil {
		return err
	}
	msg.Receiver = common.BytesToAddress(addr).String()
	k.ConvertCoin(sdk.WrapSDKContext(ctx), msg)
	return nil
}
...
func (k Keeper) PostTxProcessing(
	...
	bridge := contracts.BridgeContract.ABI

	for i, log := range receipt.Logs {
		if log.Address.String() == params.Address {

			eventID := log.Topics[0]
			event, err := bridge.EventByID(eventID)
			if err != nil {
				continue
			}

			if event.Name == types.BRIDGEBridgeEvent {
				bridgeEvent, err := bridge.Unpack(event.Name, log.Data)
				if err != nil {
					k.Logger(ctx).Error("failed to unpack bridge event", "error", err.Error())
					continue
				}
				if len(bridgeEvent) == 0 {
					continue
				}
				amount, ok := bridgeEvent[0].(*big.Int)
				if !ok || amount == nil || amount.Sign() != 1 {
					continue
				}
				evmParams := k.evmKeeper.GetParams(ctx)
				coins := sdk.Coins{{Denom: evmParams.EvmDenom, Amount: sdk.NewIntFromBigInt(amount)}}

				byteAddr := common.HexToAddress(log.Address.String()).Bytes()
				err = k.bankKeeper.SendCoinsFromAccountToModule(ctx, byteAddr, types.ModuleName, coins)
				if err != nil {
					k.Logger(ctx).Error("failed to SendCoinsFromAccountToModule", "error", err.Error())
					continue
				}
				err = k.bankKeeper.BurnCoins(ctx, types.ModuleName, coins)
				if err != nil {
					k.Logger(ctx).Error("failed to BurnCoins", "error", err.Error())
					continue
				}
			} else if event.Name == types.BRIDGEMintEvent {
				mintEvent, err := bridge.Unpack(event.Name, log.Data)
				if err != nil {
					k.Logger(ctx).Error("failed to unpack mint event", "error", err.Error())
					continue
				}
				if len(mintEvent) == 0 {
					continue
				}
				amount, ok := mintEvent[0].(*big.Int)
				if !ok || amount == nil || amount.Sign() != 1 {
					continue
				}
				evmParams := k.evmKeeper.GetParams(ctx)
				...
				if err != nil {
					k.Logger(ctx).Error("failed to MintCoins", "error", err.Error())
					continue
				}
				...
				if err != nil {
					k.Logger(ctx).Error("failed to SendCoinsFromModuleToAccount", "error", err.Error())
					continue
				}
			} else {
				continue
			...
			// Check that the contracts is a registered token pair
			...
			if !pair.Enabled {
				...
				k.Logger(ctx).Debug(
					...
					"coin", pair.Denom, "contracts", pair.Erc20Address,
				)
				continue
			}
			...
			if err != nil {
				k.Logger(ctx).Debug(
					...
					"coin", pair.Denom, "contracts", pair.Erc20Address, "error", err.Error(),
				)
				continue
			}
