...
// RegisterCoin deploys an erc20 contracts and creates the token pair for the
...
func (k Keeper) RegisterERC20(
	...
	if k.IsERC20Registered(ctx, contract) {
		return nil, sdkerrors.Wrapf(
			types.ErrTokenPairAlreadyExists, "token ERC20 contracts already registered: %s", contract.String(),
		)
	}
...
func (k Keeper) CreateCoinMetadata(
	...
	// metadata name is should always be the contracts since it's the key
	// to the bank store
	...
	if err := metadata.Validate(); err != nil {
		return nil, sdkerrors.Wrapf(
			err, "ERC20 token data is invalid for contracts %s", strContract,
		)
	}
