...
func NewConvertERC20Cmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "convert-erc20 [contracts-address] [amount] [receiver]",
...
func NewSetBridgeCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "set-bridge [address]",
		Short: "set bridge address.",
		Args:  cobra.RangeArgs(1, 2),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func NewSetBeginHeightCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "set-begin-block [height]",
		Short: "set mint begin block height.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgSetBeginBlock(sender, height)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
...
func NewFundMintCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "fund mint module",
		Short: "transfer some native coin to mint module.",
		Args:  cobra.RangeArgs(1, 1),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
			...
			msg := types.NewMsgFundMint(sender, amount)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
...

{
	...
	cmd.Flags().String(cli.FlagDeposit, "1aodx", "deposit of proposal")
...
func NewRegisterERC20ProposalCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf("$ %s tx gov submit-proposal register-erc20 <contracts-address> --from=<key_or_address>", version.AppName),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
				return err
